...
unsigned int doHash(char *str)
{
    return BKDRHash(str) % SIZE_BITMAP;
}
...
int main()
{
    ...
    while((3 == fread(head, sizeof(unsigned char),HEAD_LENGTH, pf)) && (!feof(pf)))
    {
        if(1==(int)head[0])
        {
            ...
            char kkey[70];
            sprintf(kkey,"%hi%hi%hi%hi",quat[0],quat[1],quat[2],quat[3]);
            //printf("%d %s\n", n, kkey);
            //short key = quat[0] + quat[1] + quat[2] + quat[3];
            unsigned int index = doHash(kkey);

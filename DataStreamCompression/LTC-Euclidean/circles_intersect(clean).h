...
int recursive(double left, double right, int param_index) {
    if(param_index ==0)
    {
        centre_point.coordinate[param_index] = (left+right)/2;
        return  1;
    }
    ...
    while(left < right)
    {
        ...
        for(i=0; i<tmp_list_circles.length;i++)
        {
            ...
            {
                square_dis_to_line += (centre_point.coordinate[j] - tmp_list_circles.circles[i].centre.coordinate[j]) * (centre_point.coordinate[j] - tmp_list_circles.circles[i].centre.coordinate[j]);
            }
        ...
        if(high >= low)
        {
            centre_point.coordinate[param_index] = mid;
...
int isThereIntersection(struct LIST_CIRCLE list_circle, struct CIRCLE new_circle) {
    ...
    for(i=0; i<list_circle.length; i++)
    {
        switch(relationOf2Circles(list_circle.circles[i], new_circle)){
            ...
            case INTERSECT:
                left = MAX(left, (list_circle.circles[i].centre.coordinate[param_index] - list_circle.circles[i].radius));
                right = MIN(right, (list_circle.circles[i].centre.coordinate[param_index] + list_circle.circles[i].radius));
    ...
    return recursive(left, right, param_index);
}
